...
>>>>>>> master
	...
	public static int overallDistance(int readmeDistance, int languageDistance,
		...
		return readmeDistance + languageDistance + timeDifference
				+ watchersRank;
	}
	 ...
	 * splits the string by space, and adds all the resulting words to the
	 * suffix tree. next, the algorithm uses the keywords passed in to determine
	 * the percentage of keywords that a repository has and ranks it. the
	 * keywords array should be in order of preference, with the presence of
	...
	public static int analyzeReadme(Repository repository, String[] keywords)
		...
		BufferedReader readmeReader = new BufferedReader(new InputStreamReader(
				readmeConnection.getInputStream()));
...
>>>>>>> master
	...
	/*
	 * computes the average language rank of a given user. The average language
	 * rank for a user is kind of similar to the language rank for a repository.
	 * What this method will do is total the percentage number of bytes written
	 * in each language across all of the user's public repos and then divide
	 * each running total by the number of repos. For example, suppose our user
	 * has two repos with the following ranks: { ruby = 0.5, javascript= 0.25,
	 * html = 0.25 } { ruby = 0.2, javascript= 0.3, html = 0.5 }
	 * 
	 * We would then need to divide the totals of each language by 2, so the
	 * average language rank would be { ruby = 0.35, javascript= .275, html =
	 * 0.325 }
	 */
...
>>>>>>> master
		...
		for (String language : averageLanguageRank.keySet().toArray(
			...
			averageLanguageRank.put(language, languageNumber.doubleValue()
					/ publicRepoCount);
		}
	...
	/*
	 * this method will compute the distance between the user's language rank
	 * and the language rank of a target repo. the distance will be defined as
	 * the manhattan distance between languages. So basically, we just check the
	 * two hashes and if the key exists in both hashes then we get the distance
	 * for that language as |userNumber - targetNumber|. if the key exists in
	 * one hash but not the other, then we just add the rank for that language.
	 * Finally, we multiply the sum of all the ranks by 50 to get it between 0
	 * and 100.
	 */
	public static int computeLanguageDistance(
			HashMap<String, Double> userRankToCopy,
			HashMap<String, Double> targetRank) {
		Double runningSum = 0.0;

		HashMap<String, Double> userRank = new HashMap<String, Double>(
				userRankToCopy);
		for (String language : userRank.keySet().toArray(new String[0])) {
			...
			if (targetRank.containsKey(language)) {
				...
				runningSum += Math.abs(userNumber.doubleValue()
						- targetNumber.doubleValue());
			} else {
	...
	public static List<Repository> getRandomRepositories(int limit) {
		...
		List<Repository> randomRepos = Repository
				.findBySQL("SELECT * FROM repositories OFFSET random() * "
						+ count.intValue() + " LIMIT " + limit + ";");
		return randomRepos;
	}
